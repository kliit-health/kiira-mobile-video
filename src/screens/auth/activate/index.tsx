  ...
  const CrossIcon = () => {
    return (
      <TouchableOpacity
        ...
        <Text style={styles.backText}>{login.Back}</Text>
      </TouchableOpacity>
    );
  };

  const HelloView = () => {
    return <Text style={styles.helloStyle}>{login.HelloNewMember}</Text>;
  };

  const TitleView = () => {
    return <Text style={styles.titleStyle}>{login.ActivateBelow}</Text>;
  };

  const InfoView = () => {
    return <Text style={styles.infoStyle}>{login.UseSameEmail}</Text>;
  };

  const ContentView = () => {
    return <Text style={styles.contentStyle}>{login.OrganizationPlan}</Text>;
  };
