    ...
    let filtered = expertData.filter(expert => {
      ...
      if (expert.profileInfo.profession.specialities && expert.chatEnabled) {
        if (serviceType == navItems[0].type) {
          return (
            expert.profileInfo.profession.specialities.filter(spec => {
  ...
  return (
    <Screen test="Select Chat Expert">
      <View>
        ...
        {expertData ? (
          <Favorites experts={getFavoriteExperts(favorites, experts)} />
        ) : null}
        ...
        {expertData ? (
          <Experts experts={getNonFavoriteExperts(favorites, experts)} />
        ) : null}
        {showFilter && (
          <FilterModal
            show={showFilter}
