...
const Pharmacy = ({ navigation }) => {
  ...
  const [phoneNumber, setPhoneNumber] = useState(
    user.profileInfo && user.profileInfo.phoneNumber
      ? user.profileInfo.phoneNumber
      : '',
  );
  const [address, setAddress] = useState(
    user.profileInfo && user.profileInfo.address
  ...
  return (
    <Screen test="Select Chat Expert">
      <View style={styles.headerStyle}>
        ...
        <Conditional if={!editButton}>
          <Header onBack={handleOnBackPress} title={lang.pharmacy.title} />
        </Conditional>
      </View>
      {showEdit && (
        <EditModal
      ...
      {!showEdit && (
        <View style={styles.inputTextParentContainerStyle}>
          ...
          <View style={styles.inputTextContainerStyle}>
            <View
              style={
                phoneNumber ? styles.inputTypeStyle : styles.inputEmptyTypeStyle
              }>
              <Text
                style={
                  phoneNumber
                    ? [styles.textStyle, { color: '#000B1E' }]
                    : styles.textEmptyStyle
                }>
