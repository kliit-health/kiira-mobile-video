...
const EmergencyContact = ({ navigation }) => {
  ...
  const [emergencyContactInfo, setEmergencyContactInfo] = useState(
    user.profileInfo ? user.profileInfo.emergencyContactInfo : null,
  );
  const [firstName, setFirstName] = useState(
    emergencyContactInfo && emergencyContactInfo.firstName
      ? emergencyContactInfo.firstName
      : '',
  );
  const [lastName, setLastName] = useState(
    emergencyContactInfo && emergencyContactInfo.lastName
      ? emergencyContactInfo.lastName
      : '',
  );
  const [phoneNumber, setPhoneNumber] = useState(
    emergencyContactInfo && emergencyContactInfo.phoneNumber
      ? emergencyContactInfo.phoneNumber
      : '',
  );
  const [secondNumber, setSecondNumber] = useState(
    emergencyContactInfo && emergencyContactInfo.secondNumber
      ? emergencyContactInfo.secondNumber
      : '',
  );
  const [relationship, setRelationship] = useState(
    emergencyContactInfo && emergencyContactInfo.relationship
  ...
  const toggleModal = (toggle, emergencyContactInfo) => {
    ...
    var emergency = {
      ...
      secondNumber: emergencyContactInfo.secondNumber,
  ...
  return (
    <Screen test="Select Chat Expert">
      ...
      {!showEdit && (
        <View style={styles.inputTextParentNotEditContainerStyle}>
          <View style={styles.bartellsStyle}>
            <View
              style={
                firstName
                  ? styles.inputTypeStyleNotEdit
                  : firstName
                  ? styles.noStyle
              ...
              <Text
                style={
                  firstName
                    ? [styles.textStyleNotEdit, { fontSize: 28 }]
                    : styles.textEmptyStyle
                }>
                {firstName
                  ? firstName + ' ' + lastName
                  : firstName
                  ? ''
                  : lang.emergencyContact.firstLastName}
              </Text>
            </View>
          </View>
            ...
            <View
              ...
              <Text
                style={
                  relationship
                    ? [
                        styles.textStyleNotEdit,
                        { color: '#868992', fontSize: 20, fontWeight: '400' },
                      ]
                    : styles.textEmptyStyle
                }>
                {relationship
                  ? relationship
                  : lang.emergencyContact.relationshipToYou}
              </Text>
            </View>
          </View>
          <View style={styles.inputTextContainerStyle}>
