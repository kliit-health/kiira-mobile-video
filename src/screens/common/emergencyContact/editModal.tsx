...
const EditModal = ({ show, lang, emergencyContactInfo, toggleModal }) => {
  ...
  const [lEmergencyContactInfo, setEmergencyContactInfo] =
    useState(emergencyContactInfo);
  ...
  const [firstName, setFirstName] = useState(
    emergencyContactInfo && emergencyContactInfo.firstName
      ? emergencyContactInfo.firstName + '&' + lastName
      : '',
  );
  ...
  const setValidPhoneNumber = (value, second) => {
    ...
    if (second) {
      if (
        ...
        secondNumber.substr(secondNumber.length - 1, 1) != '-'
      ) {
        value = value + '-';
      }
    } else {
      if (
        (value.length == 3 || value.length == 7) &&
        phoneNumber.substr(phoneNumber.length - 1, 1) != '-'
      ) {
        value = value + '-';
      }
    }
