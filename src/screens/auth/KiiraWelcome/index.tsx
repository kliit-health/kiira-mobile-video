...
const KiiraWelcome = ({ navigation }) => {
  ...
  const HelloView = () => {
    return <Text style={styles.helloStyle}>{welcomeKiira.welcomeKiira}</Text>;
  };

  const TitleView = () => {
    return <Text style={styles.titleStyle}>{welcomeKiira.oneStopTitle}</Text>;
  };
    ...
    <ImageBackground
      ...
      <ScrollView
        ...
        <TouchableOpacity
          ...
          onPress={() => Linking.openURL(Constant.App.becomeAMemeberUrl)}>
          <Text style={styles.becomeMember}>{welcomeKiira.becomeMember}</Text>
        </TouchableOpacity>
      </ScrollView>
