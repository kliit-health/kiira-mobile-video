...
export async function cancelAppointmentData(data, message) {
  try {
    ...
    let amount =
      data.prepaidInfo && data.prepaidInfo.amount ? data.prepaidInfo.amount : 0;
    let isPrePaid =
    ...
    const totals = {
      required: credits,
      monthly:
        userData.visits && userData.visits != 'NaN' ? userData.visits : 0,
    ...
    appointments.history = appointments.history.filter(item => item.id !== id);
