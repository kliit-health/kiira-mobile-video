...
function* getAllAppointmentDates({ payload }) {
  ...
  try {
    ...
    if (
      !response ||
      response.length == 0 ||
      response.toString().includes('TypeError')
    ) {
      yield put(hideApiLoader());
...
function* updateAppointment({ payload }) {
  ...
  try {
    ...
    if (
      assessment &&
      appointmentType &&
      appointmentType.title === 'Health Check'
    ) {
...
function* cancelTheAppointment({ payload: { data } }) {
  ...
  const isPrepaid =
    prepaidInfo && prepaidInfo.isPrePaid ? prepaidInfo.isPrePaid : false;
...
function* setAppointment({ payload }) {
  ...
  const totals = {
    ...
    redeemPrepaid: prepaid > 0 && credits - visits > 0 ? credits - visits : 0,
    redeemMonthly:
