...
const GetTreatment = ({ navigation }) => {
    ...
    const { firstName, lastName, profileImageUrl, bio, profession, languages } =
        profileInfo;
        ...
        >
            ...
            <ScrollView contentContainerStyle={styles.detailsContentContainer}>
                ...
                <Text style={styles.sectionTitle}>
                    {lang.getTreatment.about}
                </Text>
                <Text style={{ ...styles.sectionText, paddingBottom: 10 }}>
                    {bio}
                </Text>
                <Text style={styles.sectionTitle}>
                    {lang.getTreatment.languages}
                </Text>
                <Text style={styles.sectionText}>
                    {formatLanguages(languages)}
                </Text>
                <Text style={styles.sectionTitle}>
                    {lang.getTreatment.specialties}
                </Text>
                {specialities.map(speciality => (
                    <Text key={speciality} style={styles.sectionText}>
                        {speciality + ' '}
                    </Text>
                ))}
            </ScrollView>
