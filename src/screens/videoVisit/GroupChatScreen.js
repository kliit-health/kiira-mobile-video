			...
			default: {
				...
				return (
					<View
						style={[
							styles.balloon,
							{ backgroundColor: isMyMess ? "#bdbdbd" : "#3f51b5" },
							{ alignSelf: isMyMess ? "flex-end" : "flex-start" },
						]}
					>
						<Text
							style={[
								styles.item,
								{ color: isMyMess ? "#757575" : "white" },
			...
			mess => {
				...
				if (
					...
					(lastMessage.readAt == undefined &&
						lastMessage.receiverId == guid &&
						lastMessage.sender.uid !== myUserID)
				) {
					CometChat.markAsRead(
						lastMessage.id,
