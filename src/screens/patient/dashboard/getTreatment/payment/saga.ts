...
function* createPayment({ data }) {
    ...
    try {
        ...
        if (response.ok) {
            yield put(
                showOrHideModal(lang.successMessages.cardAddedSuccessfully),
            );
            navigation.goBack();
        } else {
            var errorMessage = '';
...
function* handlePayResponse(response, credits) {
    ...
    if (response.ok) {
        ...
        yield call(updateCredits, credits, {
            data: { uid: user.uid, prepaid: true },
        });
        if (response.ok) {
            const obj = {
            ...
            yield put(
                showOrHideModal(lang.successMessages.visitAddedSuccessfully),
            );
